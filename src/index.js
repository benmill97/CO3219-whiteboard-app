...
async function initialize() {
  let tools = {
    ...
    rect: {
      ...
      draw: (b, d) =>
        b
          .rect(Math.abs(d[2] - d[0]), Math.abs(d[3] - d[1]))
          .move(Math.min(d[0], d[2]), Math.min(d[1], d[3])),
      update: (e, d) =>
        e
          .x(Math.min(d[2], d[0]))
          .y(Math.min(d[1], d[3]))
          .size(Math.abs(d[2] - d[0]), Math.abs(d[3] - d[1])),
    },
    circle: {
      start: (x, y) => [x, y, 0],
      move: (x, y, d) => {
        d[2] = Math.floor(
          Math.sqrt((d[0] - x) * (d[0] - x) + (d[1] - y) * (d[1] - y))
        );
      },
      draw: (b, d) =>
        b
    ...
    ellipse: {
      ...
      draw: (b, d) =>
        b
          .ellipse(Math.abs(d[2] - d[0]), Math.abs(d[3] - d[1]))
          .cx((d[0] + d[2]) / 2)
          .cy((d[1] + d[3]) / 2),
      update: (e, d) =>
        e
          .cx((d[0] + d[2]) / 2)
          .cy((d[1] + d[3]) / 2)
          .radius(Math.abs(d[2] - d[0]) / 2, Math.abs(d[3] - d[1]) / 2),
    },
  };
  ...
  let data = await res.json();
  for (let i in data.shapes) diagram.updateShape(i, data.shapes[i]);
  if (data.background)
    diagram.updateBackground("/background/" + data.background);

  diagram.onShapeUpdate((i, m) =>
    ws.sendToGroup(
      "draw",
      {
        name: "updateShape",
  ...
  diagram.onHistoryChange(
    (p, f) => ([appData.hasUndo, appData.hasRedo] = [p, f])
  );
  ...
  let modes = {
    ...
    draw: {
      ...
      moveOne: (p, pp) => {
        if (appData.connected) diagram.drawShape(p[0], p[1]);
      },
      startTwo: () => 0,
      moveTwo: () => 0,
      end: () => {
        if (appData.connected) diagram.endShape();
      },
    },
  };
  ...
  let move = (p) => {
    ...
    if (
      !started &&
      p.length === 1 &&
      Math.abs(p[0][0] - prev[0][0]) < 5 &&
      Math.abs(p[0][1] - prev[0][1]) < 5
    )
      return;
    else {
      started = true;
  ...
  diagram.zoom(
    w < 576 ? 1 / 3 : w < 768 ? 1 / 2 : w < 992 ? 2 / 3 : w < 1200 ? 5 / 6 : 1
  );

  // hook window resize event to set correct viewbox size
  window.onresize = () =>
    diagram.resizeViewbox($("#whiteboard").width(), $("#whiteboard").height());
}
