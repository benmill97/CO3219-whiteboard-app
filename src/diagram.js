function Diagram(element, tools) {
  ...
  this._scale = 1;
  this._offset = [0, 0];
  this._shapeUpdateCallback =
    this._shapePatchCallback =
    this._shapeRemoveCallback =
    this._shapeAddCallback =
    this._clearCallback =
    this._historyChangeCallback =
      () => {};
}
...
Diagram.prototype.startShape = function (k, c, w, x, y) {
  ...
  this._shapes[this._id] = {
    view: applyStyle(this._tools[k].draw(this._element, m.data), c, w),
    model: m,
  };
  this._future = [];
...
Diagram.prototype.updateShape = function (i, m) {
  if (this._shapes[i])
    this._tools[m.kind].update(
      this._shapes[i].view,
      (this._shapes[i].model.data = m.data)
    );
  else
    this._shapes[i] = {
      view: applyStyle(
        this._tools[m.kind].draw(this._element, m.data),
        m.color,
        m.width
      ),
      model: m,
...
Diagram.prototype.patchShape = function (i, d) {
  if (this._shapes[i])
    this._tools[this._shapes[i].model.kind].update(
      this._shapes[i].view,
      (this._shapes[i].model.data = this._shapes[i].model.data.concat(d))
    );
};
